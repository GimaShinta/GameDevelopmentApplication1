...
protected:
	...
	std::vector<eEnemyDirectionState> state;
...
protected:
	eEnemyDirectionState next_direction;      //今の進行方向
	eEnemyDirectionState now_direction;      //前の進行方向
...
protected:
	...
	bool end_anim;                           //イジケ終了のアニメーションに変更させる

	const int izike_num[2] = { 16, 17 };                  //イジケ状態アニメーションの順番
	const int izikeend_num[4] = { 16, 17, 18, 19 };       //イジケ終了状態アニメーション順番
...
public:
	...
	void SetPlayerControl(class Player* player);
	int PanelDistance(int a, int b);
};